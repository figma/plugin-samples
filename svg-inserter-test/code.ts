...
function getIcon(name: string, size: number, color?: string): string | null {
  ...
  color = color ?? 'black'
  return svg.replace(/\$size\$/g, size).replace(/\$color\$/g, color)
}
...
figma.parameters.on('input', (parameters: ParameterValues, currentKey: string, result: SuggestionResults) => {
  ...
  switch (currentKey) {
    case 'icon-name':
      ...
      const suggestions: SuggestionExtended[] = iconNames.filter(s => s.includes(query)).map((name, index) => {
        return {
          name,
          icon: getIcon(name, 150),
