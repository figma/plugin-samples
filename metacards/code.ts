...
if (selectedTextNodes.length) {
  const textNode = selectedTextNodes[0];

  const hyperLinks = getHyperlinksFromTextNode(textNode);
...
function getHyperlinksFromTextNode(textNode) {
  const links = Array.from(textNode.characters).map((char, i) => {
    const link = textNode.getRangeHyperlink(i, i + 1);
...
figma.ui.onmessage = async (msg) => {
  ...
  if (msg.function === "fetchError") {
    ...
    // Create Image
    const img = figma.createImage(metaTags.image.data);
    ...
    // Create title
    const metaTitle = figma.createText();
    ...
    // Create description
    const metaDesc = figma.createText();
    ...
    cardFrame.y = node.y;
    node.parent.appendChild(cardFrame);
